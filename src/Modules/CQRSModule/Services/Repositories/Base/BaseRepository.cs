...
namespace CQRSModule.Services.Repositories.Base;
    ...
    where TDbContext : DbContext
{
    ...
    public async Task<int> CountAsync(Expression<Func<TEntity, bool>>? predicate = null, bool tracking = false,
        CancellationToken cancellationToken = default)
    {
        IQueryable<TEntity> query = DatabaseSet;
        if (!tracking) query = query.AsNoTracking();
        if (predicate == null) return await query.CountAsync(cancellationToken).ConfigureAwait(false);
        return await query.CountAsync(predicate, cancellationToken).ConfigureAwait(false);
    }

    public async Task<bool> ExistsAsync(Expression<Func<TEntity, bool>> predicate, bool tracking = false,
        CancellationToken cancellationToken = default)
    {
        IQueryable<TEntity> query = DatabaseSet;
        if (!tracking) query = query.AsNoTracking();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (selector != null) return await query.Select(selector).ToListAsync(cancellationToken).ConfigureAwait(false);
    ...
    {
        var match = await DatabaseSet.FindAsync(id).ConfigureAwait(false);
    ...
    public async Task<TEntity> UpdateAsync(TEntity entity, CancellationToken cancellationToken = default)
    {
        ...
        var keyProperty = (from property in properties
            let keyAttr = property.GetCustomAttributes(typeof(KeyAttribute), false).Cast<KeyAttribute>()
                .FirstOrDefault()
            where keyAttr != null
        ...
        var result = await DatabaseSet.FindAsync(keyProperty.GetValue(entity)).ConfigureAwait(false);

        if (result == null) throw new ArgumentException($"{entity.GetType()} does not exists", nameof(entity));
